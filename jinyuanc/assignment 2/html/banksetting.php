...
if(isset($_POST['submit'])) {
    ...
    //serach information from database
    $query = "SELECT * FROM BankAccounts WHERE account = '$session_user'";

    $result = $mysqli->query($query);
    if($row = $result->fetch_array(MYSQLI_ASSOC)) {
        //if we have found the result, save the name and saving     amount and other information
        $email = $row['email'];
        $phone = $row['phone'];
        $password = $row['password'];
    }
    
    //judge the password and newpassword
    if($newpassword == "") {
        $error = "Sorry, your have to input your new password.";
    } else if ($password != $oldpassword) {
        $error = "Sorry, your old password is wrong.<br><br>Please input again.";
        ...
        $error = "Sorry, your password must be included at least one letter.";
    } else if (!preg_match("#[A-Z]+#", $newpassword)) {
        $error = "Sorry, your password must be included at least one upper letter.";
    } else if (!preg_match("#[0-9]+#", $newpassword)) {
        $error = "Sorry, your password must be included at least one number.";
    } /*else if(!preg_match("#[~!@#$%^&]+#", $newpassword)) {
        $error = "Sorry, your password mush be included<br><br>at least one of the following characters ~!@#$%^&.";
    }*/
    
     //if the passwords no any problem, update the password in the database
     else {
        $query = "UPDATE BankAccounts SET password = '$newpassword' WHERE account = '$session_user'";
        $result = $mysqli->query($query);
        $error = "You have updated your password.";
    }
}
    ...
    <div id="all">
        ...
        <div class="content" id="class2">
            <div id="middle" class="middle" style="text-align:center;">
                <h2 style="margin-top:22%">Welcome, <?php echo $session_user; ?></h2>
                <form method="post" action="">
                    <p>You can update your password here:</p>
                    <p>Old password: <input type="text" name="password"></p>
